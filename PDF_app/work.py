...
def init_llm():
    ...
    llm = ChatOpenAI(model_name="your_model", openai_api_key = api_key)
    # Initialize the embeddings for the language model
    llm_embeddings = OpenAIEmbeddings(openai_api_key = api_key)
    ##response = self.client.create_chat_completion(prompt=_prompts, **params)
...
def process_prompt(prompt):
    ...
    result = conversation_retrieval_chain.invoke({"question": prompt, "chat_history": chat_history})
